<template>
  <div class="slider-page container-fluid">
   ...
   <div class="pagination-container" v-if="sliders.length > 10">
      <ul class="pagination">
        <li class="page-item">
...
export default {
  ...
  computed: {
    ...
    return this.sliders.slice(startIndex, endIndex);

  },
  totalPages() {
  return Math.ceil(this.sliders.length / this.itemsPerPage);
},
  ...
  methods: {
    ...
    async getSlider() {
      ...
      .get(`/slider?limit=${this.limit}&page=${this.page}`) // Use backticks here
        .then((res) => res.data);
  ...
  await this.getSlider(); // Call getSlider to fetch data
},
